# Lineage MP Timer — 버전 히스토리 아카이브 (v1.0.0 ~ v2.0.0)

> v3.0(2026-06-05, TypeScript + electron-vite 전면 리뉴얼) **이전**의 상세 변경 이력입니다.
> 이 블록은 원래 `CLAUDE.md`에 인라인되어 있었으나(약 1030줄), 매 세션 컨텍스트 부담을 줄이기 위해 분리했습니다.
> v3.0 이후 변경은 `CHANGELOG.md` 참고. 대부분의 라인 참조(`app.js:NNNN` 등)는 v3.0에서 삭제된 코드 기준입니다.
