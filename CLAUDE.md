...
1. **MP 바 픽셀 측정** (`bar-fill.ts`) — 게이지 채움 컬럼 직접 셈. OCR 불필요, ~100% 정확(100% 1회 보정 필요). MP 1순위. **Robustness(v3.0.1)**: ① 빈-트랙 색을 바 우측에서 자동 샘플링해 컬럼을 *상대 거리*(채움색 vs 트랙색)로 분류 → 채움/빈 색이 비슷해도 고착(saturation) 방지(가득 차 빈 샘플 없으면 절대-허용오차 폴백). ② 경계는 *오른쪽에서부터* "국소 채움 우세 최우측" 탐색 → 스킬 이펙트/플로팅 데미지 등 내부 가림이 측정을 자르는 거짓 급락 차단, 우측 노이즈는 국소밀도 게이트로 거름.
